...
from models.backbone.efficientnetv2.efficientnet_v2_config import get_efficientnet_v2_structure
from models.backbone.efficientnetv2.pretrained_weight_loader import load_from_zoo
...
def efficientnet_v2s(pretrained=False):
    return get_efficientnet_v2(model_name='efficientnet_v2_s_in21k',pretrained=pretrained,dropout=0.2)

def efficientnet_v2m(pretrained=False):
    return get_efficientnet_v2(model_name='efficientnet_v2_m',pretrained=pretrained,dropout=0.3)

def efficientnet_v2l(pretrained=False):
    return get_efficientnet_v2(model_name='efficientnet_v2_l',pretrained=pretrained,dropout=0.4)
if __name__ == '__main__':

    from torchsummaryX import summary
    model = efficientnet_v2s(True)
    model2 = efficientnet_v2s(False)
    print(len(model.blocks))
    model_dict = model.state_dict()
    model_dict2 = model2.state_dict()
    # print(model_dict.items())
    # print(model_dict2.items())
    a = 0
    b=0
    for i, (k, v) in enumerate(model_dict.items()):
        ...
        if not torch.equal(model_dict[k], model_dict2[k]) :
            a+=1
