...
from models.backbone.fastVit.modules.mobileone import MobileOneBlock
from models.backbone.fastVit.modules.replknet import ReparamLargeKernelConv
...
def fastvit_sa12(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        root = r'/usr/local/data/guopeng/PointNu-Net-main/models/pretraned/fastvit/fastvit_sa12.pth.tar'
...
def fastvit_sa24(pretrained=False, **kwargs):
    ...
    if pretrained:
        print('loading pretrained model......')
        root = r'/usr/local/data/guopeng/PointNu-Net-main/models/pretraned/fastvit/fastvit_sa24.pth.tar'
        old_dict = torch.load(root)
        model.load_state_dict(old_dict['state_dict'])
    return model
...
def fastvit_sa36(pretrained=False, **kwargs):
    ...
    if pretrained:
        # raise ValueError("Functionality not implemented.")
        print('loading pretrained model......')
        root = r'/usr/local/data/guopeng/PointNu-Net-main/models/pretraned/fastvit/fastvit_sa36.pth.tar'
        old_dict = torch.load(root)
        model.load_state_dict(old_dict['state_dict'])
    return model
...
def fastvit_ma36(pretrained=False, **kwargs):
    ...
    if pretrained:
        print('loading pretrained model......')
        # raise ValueError("Functionality not implemented.")
        root = r'/usr/local/data/guopeng/PointNu-Net-main/models/pretraned/fastvit/fastvit_ma36.pth.tar'
        old_dict = torch.load(root)
        model.load_state_dict(old_dict['state_dict'])
        # model_dict = model.state_dict()
        # print(len(old_dict['state_dict'].items()))
        # print(len(model_dict.items()))
        # for i, (k, v) in enumerate(old_dict['state_dict'].items()):
        #     if k not in model_dict:
        #         print(k)
        #         pass
        # old_dict = {k: v for k, v in old_dict['state_dict'].items() if (k in model_dict)}
...
if __name__ == '__main__':
    ...
    model = fastvit_sa12(pretrained=True)
    model2 = fastvit_sa12(pretrained=False)
    # print(model.network[6])
    ...
    #     x = block(x)
    #     if idx in [0,2,4,7]:
    #         bn = nn.BatchNorm2d(num_features=x.shape[1])
    #         x = bn(x)
    #         outs.append(x)
    # for i in outs:
    #     print(i.shape)
    ...
    for i, (k, v) in enumerate(model_dict.items()):
        b += 1
        if k not in model_dict2:
            print(k)
            pass
        if torch.equal(model_dict[k], model_dict2[k]):
            a += 1
