...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        res = self.avgpool(x)
        res = self.ca(res)
        res = res.unsqueeze(2).unsqueeze(3).expand_as(x)
        return res
...
class BAMBlock(nn.Module):
    ...
    def __init__(self, channel=512, reduction=16, dia_val=4):
        super().__init__()
...
if __name__ == '__main__':
    input = torch.randn(2, 64, 64, 64)
    bam = BAMBlock(channel=64, reduction=16, dia_val=4)
    output = bam(input)
    print(output.shape)
